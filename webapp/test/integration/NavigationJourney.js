sap.ui.define(['sap/ui/test/opaQunit'], function(opaTest) {
  ...
  opaTest('Object Page shows the correct object Details', function(
    Given,
    When,
    Then
  ) {
  ...
  opaTest(
    'Should be on the table page again when browser back is pressed',
  ...
  opaTest(
    'Should be on the object page again when browser forwards is pressed',
    function(Given, When, Then) {
      // Actions
  ...
  opaTest('Should see a busy indication while loading the metadata', function(
    Given,
    When,
    Then
  ) {
    ...
    // Assertions
    Then.onTheAppPage
      .iShouldSeeTheBusyIndicatorForTheWholeApp()
      .and.iTeardownMyAppFrame()
  })

  opaTest(
    'Start the App and simulate metadata error: MessageBox should be shown',
    function(Given, When, Then) {
      //Arrangement
      Given.iStartMyAppOnADesktopToTestErrorHandler('metadataError=true')

      //Assertions
      Then.onTheAppPage.iShouldSeeTheMessageBox().and.iTeardownMyAppFrame()
    }
  )

  opaTest(
    'Start the App and simulate bad request error: MessageBox should be shown',
    function(Given, When, Then) {
      //Arrangement
